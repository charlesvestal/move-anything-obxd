...
/* Output gain - reduced to prevent clipping with hot presets */
static float g_output_gain = 0.5f;
...
static void v2_apply_param(obxd_instance_t *inst, int bank, int idx, float value) {
    ...
    switch (bank) {
        ...
        case 1:
            switch (idx) {
                case 0:
                    synth->processOsc1Saw(value > 0.5f ? 1.0f : 0.0f);
                    synth->processOsc1Pulse(value <= 0.5f ? 1.0f : 0.0f);
                    break;
                case 1: synth->processPulseWidth(value); break;
                case 2:
                    synth->processOsc2Saw(value > 0.5f ? 1.0f : 0.0f);
                    synth->processOsc2Pulse(value <= 0.5f ? 1.0f : 0.0f);
                    break;
                case 3: synth->processPulseWidth(value); break;
                case 4: synth->processOsc2Det(value); break;
                case 5:
                    synth->processOsc1Mix(1.0f - value);
                    synth->processOsc2Mix(value);
                    break;
                case 6: synth->processOsc2Pitch(value); break;
                case 7: synth->processNoiseMix(value); break;
            }
            break;
        case 2:
            switch (idx) {
                ...
                case 1:
                    synth->processLfoSine(value < 0.33f ? 1.0f : 0.0f);
                    synth->processLfoSquare(value >= 0.33f && value < 0.66f ? 1.0f : 0.0f);
                    synth->processLfoSH(value >= 0.66f ? 1.0f : 0.0f);
                    break;
                case 2: synth->processLfoFilter(value > 0.5f ? 1.0f : 0.0f); break;
                case 3:
                    synth->processLfoOsc1(value > 0.5f ? 1.0f : 0.0f);
                    synth->processLfoOsc2(value > 0.5f ? 1.0f : 0.0f);
                    break;
                case 4:
                    synth->processLfoPw1(value > 0.5f ? 1.0f : 0.0f);
                    synth->processLfoPw2(value > 0.5f ? 1.0f : 0.0f);
                    break;
                case 5: synth->procModWheelFrequency(value); break;
...
static int v2_load_bank(obxd_instance_t *inst, const char *bank_path) {
    ...
    free(data);
    fprintf(stderr, "OB-Xd v2: Loaded %d presets from bank\n", inst->preset_count);
...
static void* v2_create_instance(const char *module_dir, const char *json_defaults) {
    ...
    fprintf(stderr, "OB-Xd v2: Instance created\n");
    return inst;
}
...
static void v2_destroy_instance(void *instance) {
    ...
    free(inst);
    fprintf(stderr, "OB-Xd v2: Instance destroyed\n");
}
        ...
        case 0xB0:
            switch (data1) {
                ...
                case 64:
                    ...
                    break;
                case 123: inst->synth->allNotesOff(); break;
            }
            break;
        case 0xE0:
            {
                int bend = ((int)data2 << 7) | data1;
                float normalized = (bend - 8192) / 8192.0f;
                inst->synth->procPitchWheel(normalized * 0.5f + 0.5f);
            }
            break;
    }
}
...
static void v2_set_param(void *instance, const char *key, const char *val) {
    ...
    if (strcmp(key, "param_bank") == 0) {
        ...
        int new_preset = (int)fval;
        if (new_preset >= 0 && new_preset < inst->preset_count) {
            inst->synth->allNotesOff();
            inst->current_preset = new_preset;
            v2_apply_preset(inst, inst->current_preset);
        }
    } else if (strcmp(key, "all_notes_off") == 0) {
        inst->synth->allNotesOff();
    } else if (strcmp(key, "panic") == 0) {
        inst->synth->allSoundOff();
    } else {
        for (int bank = 0; bank < 3; bank++) {
            for (int idx = 0; idx < 8; idx++) {
                if (strcmp(key, g_param_names[bank][idx]) == 0) {
                    v2_apply_param(inst, bank, idx, fval);
                    return;
                }
            }
        }
...
static int v2_get_param(void *instance, const char *key, char *buf, int buf_len) {
    ...
    if (strcmp(key, "preset_name") == 0 || strcmp(key, "name") == 0) {
        ...
        for (int bank = 0; bank < 3; bank++) {
            for (int idx = 0; idx < 8; idx++) {
                if (strcmp(key, g_param_names[bank][idx]) == 0) {
                    return snprintf(buf, buf_len, "%.3f", inst->params[bank * 8 + idx]);
                }
            }
        }
    }
...
extern "C" plugin_api_v2_t* move_plugin_init_v2(const host_api_v1_t *host) {
    ...
    fprintf(stderr, "OB-Xd v2 API initialized\n");
